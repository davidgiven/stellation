...
.program $god $player:http_preferences_single tnt
	{c, method, param} = args;
	{?name=player.name, ?empire=player.empire, ?email=player.email, ?mapwidth=tostr(player.mapwidth), ?mapheight=tostr(player.mapheight), ?mapdefaultscale=tostr(player.mapdefaultscale), ?mapaspect=tostr(player.mapaspect)} = $http_server:parseparam(param, {"name", "empire", "email", "mapwidth", "mapheight", "mapdefaultscale", "mapaspect"});
